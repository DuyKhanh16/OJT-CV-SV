...
export class EducationCandidateController {
  constructor(
    private readonly educationCandidateService: EducationCandidateService,
  ) {}
  ...
  create(@Body() body: CreateEducationCandidateDto, @Res() res) {
    console.log(body);
    try {
      const { candidate_id, name_education, major, start_at, end_at, info } =
        body;
      // console.log(body)
      const result = this.educationCandidateService.createNewEducation(
        candidate_id,
        name_education,
        major,
        start_at,
  ...
  update(
    @Body() body: CreateEducationCandidateDto,
    @Res() res,
    @Param('id') id: string,
  ) {
    try {
      const { candidate_id, name_education, major, start_at, end_at, info } =
        body;
      // console.log(body)
      const result = this.educationCandidateService.updateNewEducation(
        id,
        candidate_id,
        name_education,
        major,
