...
export class CandidatesController {
  ...
  async findAllInformation(@Res() res, @Req() req) {
    ...
    try {
      const result = await this.candidatesService.getAllInformation(
        req.account.email,
      );
      res.status(200).json({
        message: 'success',
  ...
  async updateAboutMe(
    ...
    try {
      const result = await this.candidatesService.updateAboutMe(
        body.aboutMe,
        req.account.email,
      );
  ...
  @UseGuards(AuthGuard)
  async updateInfoCandidate(
    @Body() body: UpdateInforCandidateDto,
    @Res() res,
    @Req() req,
  ) {
    // console.log(req.account.email)
    ...
    console.log(body);
    try {
      const result = await this.candidatesService.updateInfoCandidate(
        body,
        req.account.email,
      );
      res.status(200).json({ message: 'Cập nhật thành công' });
  ...
  async searchCandidate(
    ...
    try {
      const result = await this.candidatesService.searchCandidate(
        name,
        location,
        level,
        position,
  ...
  async candidateSaveJob(
    ...
    try {
      await this.candidatesService.createSaveCandidateJob(
        req.account.email,
        job_id,
      );
      res
  ...
  async getJobSaveCandidate(@Res() res, @Req() req) {
    try {
      const result = await this.candidatesService.getJobSaveCandidate(
        req.account.email,
      );
  ...
  async checkSaveJob(@Res() res, @Req() req, @Query('job_id') job_id) {
    const result = await this.candidatesService.checkSaveJob(
      req.account.email,
      job_id,
    );
    if (result) {
